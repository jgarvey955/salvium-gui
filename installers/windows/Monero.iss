...
DefaultDirName={commonpf}\Salvium GUI Wallet
DefaultGroupName=Salvium GUI Wallet
UninstallDisplayIcon={app}\salvium-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=Salvium
AppPublisherURL=https://salvium.io
TimeStampsInUTC=yes
...
; Salvium GUI wallet exe and guide
Source: "bin\salvium-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\salvium-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; Salvium CLI wallet
Source: "bin\extras\salvium-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\salvium-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; Salvium wallet RPC interface implementation
Source: "bin\extras\salvium-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Salvium daemon
Source: "bin\salviumd.exe"; DestDir: "{app}"; Flags: ignoreversion

; Salvium daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "salvium-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Salvium blockchain utilities
Source: "bin\extras\salvium-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\salvium-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\salvium-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\salvium-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\salvium-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\salvium-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\salvium-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\salvium-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\salvium-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\salvium-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\salvium-gen-ssl-cert.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\salvium-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\salvium');
  s := 'The default folder to store the Salvium blockchain is ' + blockChainDefaultDir;
  s := s + '. As this will need more than 10 GB of free space, you may want to use a folder on a different drive.';
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\salvium\salvium-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Salvium default location
...
begin
  Result := BlockChainDir('') + '\salvium.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "salvium-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Salvium daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\salviumd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\salvium-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Salvium GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\salvium-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\salvium-gui-wallet-guide.pdf"; IconFilename: "{app}\salvium-wallet-gui.exe"
...
Name: "{group}\Utilities\Salvium Daemon"; Filename: "{app}\salviumd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\salvium-wallet-cli.exe"; WorkingDir: "{userdocs}\Salvium\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\Salvium\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\salvium-wallet-gui\salvium-wallet-gui.log"""
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\salvium-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM salviumd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\salvium-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\salvium"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\salvium\salvium-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\salvium\salvium-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "salvium:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "salvium://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "salvium"; ValueType: "string"; ValueData: "URL:Salvium Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "salvium"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "salvium\DefaultIcon"; ValueType: "string"; ValueData: "{app}\salvium-wallet-gui.exe,0"
Root: HKCR; Subkey: "salvium\shell\open\command"; ValueType: "string"; ValueData: """{app}\salvium-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "salviumseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "salviumseed://a.b.c.d:port"
Root: HKCR; Subkey: "salviumseed"; ValueType: "string"; ValueData: "URL:Salvium Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "salviumseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "salviumseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\salvium-wallet-gui.exe,0"
Root: HKCR; Subkey: "salviumseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\salvium-wallet-gui.exe"" ""%1"""
